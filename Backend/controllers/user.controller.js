...
export const register = async (req, res) => {
  try {
    ...
    if (!fullname || !email || !password) return res.status(400).json({ message: "All fields are require", success: false });
...
export const login = async (req, res) => {
  try {
    ...
    if (!email || !password) return res.status(400).json({ message: "All fields are require", success: false });
    ...
    const token = await jwt.sign(tokenData, process.env.SECRET.KEY, { expiredIn: '1d' });
